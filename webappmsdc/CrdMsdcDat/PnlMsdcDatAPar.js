/**
  * \file PnlMsdcDatAPar.js
  * web client functionality for panel PnlMsdcDatAPar
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppMsdcDatAPar", "srefIxMsdcVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppMsdcDatAPar", "srefIxMsdcVExpstate", "mind");
	...
	// change content
	doc.getElementById("APar_side").src = "./PnlMsdcDatAPar_aside.html";
	doc.getElementById("APar_cont").src = "./PnlMsdcDatAPar_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppMsdcDatAPar", "srefIxMsdcVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppMsdcDatAPar", "srefIxMsdcVExpstate", "regd");
	...
	// change content
	doc.getElementById("APar_side").src = "./PnlMsdcDatAPar_bside.html";
	doc.getElementById("APar_cont").src = "./PnlMsdcDatAPar_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcDatAPar", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagMsdcDatAPar", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxMsdcVExpstate = retrieveSi(srcdoc, "StatAppMsdcDatAPar", "srefIxMsdcVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcDatAPar", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryMsdcDatAPar", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcDatAPar", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcDatAPar", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcDatAPar", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcDatAPar", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcDatAPar", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcDatAPar", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcDatAPar", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcDatAPar", "ncol"));

		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcDatAPar", "TcoStaWidth"));
		var TcoKeyWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcDatAPar", "TcoKeyWidth"));
		var TcoFilWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcDatAPar", "TcoFilWidth"));
		var TcoValWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcDatAPar", "TcoValWidth"));
		var TcoBinWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcDatAPar", "TcoBinWidth"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				...
				doc.cols.push("TcoBin"); doc.widths.push(TcoBinWidth); doc.fs.push("bin"); wtot += TcoBinWidth;
			} else {
				ix--;
			};
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagMsdcDatAPar", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryMsdcDatAPar", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListMsdcQDatAPar
		rownode = getRownode(srcdoc, "ListMsdcQDatAPar", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagMsdcDatAPar", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagMsdcDatAPar", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfMsdcDatAPar", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcDatAPar", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcDatAPar", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcDatAPar", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryMsdcDatAPar", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryMsdcDatAPar", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppMsdcDatAParData", scrJref, "StgIacQryMsdcDatAPar");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppMsdcDatAPar", "srefIxMsdcVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcDatAPar", "firstcol"));

	setSi(srcdoc, "StatAppQryMsdcDatAPar", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppMsdcDatAPar", "srefIxMsdcVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcDatAPar", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppMsdcDatAParDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngMsdcDatAParData", "ContInfMsdcDatAPar", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngMsdcDatAParData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngMsdcDatAParData", "StatAppMsdcDatAPar", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngMsdcDatAParData", "StatShrMsdcDatAPar", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngMsdcDatAParData", "StgIacMsdcDatAPar", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngMsdcDatAParData", "TagMsdcDatAPar", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngMsdcDatAParData", "ListMsdcQDatAPar", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngMsdcDatAParData", "StatAppQryMsdcDatAPar", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngMsdcDatAParData", "StatShrQryMsdcDatAPar", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngMsdcDatAParData", "StgIacQryMsdcDatAPar", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngMsdcDatAParData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngMsdcDatAParData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngMsdcDatAParData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
