...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppMsdcDatRef1NFile", "srefIxMsdcVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppMsdcDatRef1NFile", "srefIxMsdcVExpstate", "mind");
	...
	// change content
	doc.getElementById("Ref1NFile_side").src = "./PnlMsdcDatRef1NFile_aside.html";
	doc.getElementById("Ref1NFile_cont").src = "./PnlMsdcDatRef1NFile_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppMsdcDatRef1NFile", "srefIxMsdcVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppMsdcDatRef1NFile", "srefIxMsdcVExpstate", "regd");
	...
	// change content
	doc.getElementById("Ref1NFile_side").src = "./PnlMsdcDatRef1NFile_bside.html";
	doc.getElementById("Ref1NFile_cont").src = "./PnlMsdcDatRef1NFile_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcDatRef1NFile", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagMsdcDatRef1NFile", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxMsdcVExpstate = retrieveSi(srcdoc, "StatAppMsdcDatRef1NFile", "srefIxMsdcVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcDatRef1NFile", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryMsdcDatRef1NFile", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcDatRef1NFile", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcDatRef1NFile", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcDatRef1NFile", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcDatRef1NFile", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcDatRef1NFile", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcDatRef1NFile", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcDatRef1NFile", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcDatRef1NFile", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcDatRef1NFile", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagMsdcDatRef1NFile", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryMsdcDatRef1NFile", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListMsdcQDatRef1NFile
		rownode = getRownode(srcdoc, "ListMsdcQDatRef1NFile", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagMsdcDatRef1NFile", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagMsdcDatRef1NFile", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfMsdcDatRef1NFile", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcDatRef1NFile", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcDatRef1NFile", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcDatRef1NFile", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryMsdcDatRef1NFile", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryMsdcDatRef1NFile", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppMsdcDatRef1NFileData", scrJref, "StgIacQryMsdcDatRef1NFile");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppMsdcDatRef1NFile", "srefIxMsdcVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcDatRef1NFile", "firstcol"));

	setSi(srcdoc, "StatAppQryMsdcDatRef1NFile", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppMsdcDatRef1NFile", "srefIxMsdcVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcDatRef1NFile", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppMsdcDatRef1NFileDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngMsdcDatRef1NFileData", "ContInfMsdcDatRef1NFile", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngMsdcDatRef1NFileData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngMsdcDatRef1NFileData", "StatAppMsdcDatRef1NFile", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngMsdcDatRef1NFileData", "StatShrMsdcDatRef1NFile", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngMsdcDatRef1NFileData", "StgIacMsdcDatRef1NFile", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngMsdcDatRef1NFileData", "TagMsdcDatRef1NFile", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngMsdcDatRef1NFileData", "ListMsdcQDatRef1NFile", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngMsdcDatRef1NFileData", "StatAppQryMsdcDatRef1NFile", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngMsdcDatRef1NFileData", "StatShrQryMsdcDatRef1NFile", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngMsdcDatRef1NFileData", "StgIacQryMsdcDatRef1NFile", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngMsdcDatRef1NFileData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngMsdcDatRef1NFileData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngMsdcDatRef1NFileData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
