/**
  * \file CrdMsdcDat.js
  * web client functionality for card CrdMsdcDat
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppMsdcDat", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppMsdcDat", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppMsdcDat", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppMsdcDat", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppMsdcDat", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlMsdcDatHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlMsdcDatList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlMsdcDatRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppMsdcDat", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppMsdcDat", "shortMenu", short);
	setSi(srcdoc, "StatAppMsdcDat", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppMsdcDat", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppMsdcDat", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagMsdcDat", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagMsdcDat", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	setTextContent(mendoc, mendoc.getElementById("MtxCrdDat"), retrieveCi(srcdoc, "ContInfMsdcDat", "MtxCrdDat"));
};
...
function refresh() {
	...
	doc.title = retrieveCi(srcdoc, "ContInfMsdcDat", "MtxCrdDat") + " - MultispectralDetectorControl 0.1.52";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppMsdcDatDo", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppMsdcDatDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfMsdcDat", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngMsdcDatData", "ContInfMsdcDat", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngMsdcDatData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngMsdcDatData", "StatAppMsdcDat", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngMsdcDatData", "StatShrMsdcDat", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngMsdcDatData", "TagMsdcDat", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngMsdcDatData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrMsdcDat", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrMsdcDat", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrMsdcDat", "scrJrefRec");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngMsdcDatData") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxMsdcVReqitmode = retrieveSi(srcdoc, "StatAppMsdcDat", "srefIxMsdcVReqitmode");

	if (srefIxMsdcVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxMsdcVReqitmode = "notify";
		setSi(srcdoc, "StatAppMsdcDat", "srefIxMsdcVReqitmode", srefIxMsdcVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxMsdcVReqitmode = retrieveSi(srcdoc, "StatAppMsdcDat", "srefIxMsdcVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppMsdcDat", "latency"));
