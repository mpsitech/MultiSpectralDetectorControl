/**
  * \file PnlMsdcDatList.js
  * web client functionality for panel PnlMsdcDatList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlMsdcDatList_aside.html";
	doc.getElementById("List_cont").src = "./PnlMsdcDatList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlMsdcDatList_bside.html";
	doc.getElementById("List_cont").src = "./PnlMsdcDatList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcDatList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagMsdcDatList", "Trs"));
...
function init() {
	var srefIxMsdcVExpstate = retrieveSi(srcdoc, "StatShrMsdcDatList", "srefIxMsdcVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfMsdcDatList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagMsdcDatList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcDatList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryMsdcDatList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcDatList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfMsdcDatList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryMsdcDatList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcDatList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcDatList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcDatList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcDatList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcDatList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcDatList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagMsdcDatList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfMsdcDatList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcDatList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcDatList", "ncol"));

		var TcoAftWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcDatList", "TcoAftWidth"));
		var TcoPrdWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcDatList", "TcoPrdWidth"));
		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcDatList", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcDatList", "TcoStoWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacMsdcDatList", "numFTos"));
		...
		for (i=firstcol,ix=0 ; i<=ncol ; i++) {
			...
			if (i == 1) {
				doc.cols.push("TcoAft"); doc.widths.push(TcoAftWidth); doc.fs.push("aft2"); doc.tos.push(0); wtot += TcoAftWidth;
			} else if (i == 2) {
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagMsdcDatList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryMsdcDatList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListMsdcQDatList
		rownode = getRownode(srcdoc, "ListMsdcQDatList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagMsdcDatList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagMsdcDatList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfMsdcDatList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcDatList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcDatList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcDatList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryMsdcDatList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryMsdcDatList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppMsdcDatListData", scrJref, "StgIacQryMsdcDatList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcDatList", "firstcol"));

	setSi(srcdoc, "StatAppQryMsdcDatList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcDatList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppMsdcDatListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacMsdcDatList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacMsdcDatList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppMsdcDatListData", scrJref, "ContIacMsdcDatList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngMsdcDatListData", "ContIacMsdcDatList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngMsdcDatListData", "ContInfMsdcDatList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngMsdcDatListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngMsdcDatListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngMsdcDatListData", "StatShrMsdcDatList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngMsdcDatListData", "StgIacMsdcDatList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngMsdcDatListData", "TagMsdcDatList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngMsdcDatListData", "ListMsdcQDatList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngMsdcDatListData", "StatAppQryMsdcDatList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngMsdcDatListData", "StatShrQryMsdcDatList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngMsdcDatListData", "StgIacQryMsdcDatList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngMsdcDatListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngMsdcDatListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngMsdcDatListData") {
				mask = mergeDpchEngData(dom);
