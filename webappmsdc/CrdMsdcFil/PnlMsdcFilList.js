/**
  * \file PnlMsdcFilList.js
  * web client functionality for panel PnlMsdcFilList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlMsdcFilList_aside.html";
	doc.getElementById("List_cont").src = "./PnlMsdcFilList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlMsdcFilList_bside.html";
	doc.getElementById("List_cont").src = "./PnlMsdcFilList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcFilList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagMsdcFilList", "Trs"));
...
function init() {
	var srefIxMsdcVExpstate = retrieveSi(srcdoc, "StatShrMsdcFilList", "srefIxMsdcVExpstate");
...
function refreshA() {
	var TxtFor = retrieveCi(srcdoc, "ContInfMsdcFilList", "TxtFor");

	setTextContent(contdoc, contdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagMsdcFilList", "TxtFor"), TxtFor));

	if (TxtFor == "") {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcFilList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryMsdcFilList", "ntot") + ")");
		clearElem(contdoc.getElementById("TxtPre"));
	} else {
		setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcFilList", "Cpt"));
		setTextContent(contdoc, contdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfMsdcFilList", "TxtPre") + " (" + retrieveSi(srcdoc, "StatShrQryMsdcFilList", "ntot") + ")");
	};
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcFilList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcFilList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcFilList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcFilList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcFilList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcFilList", "ntot"));
	...
	// header row
	setTextContent(hdrdoc, hdrdoc.getElementById("TxtFor"), strFor(retrieveTi(srcdoc, "TagMsdcFilList", "TxtFor"), TxtFor));

	if (TxtFor == "") clearElem(hdrdoc.getElementById("TxtPre"));
	else setTextContent(hdrdoc, hdrdoc.getElementById("TxtPre"), retrieveCi(srcdoc, "ContInfMsdcFilList", "TxtPre"));

	// update column characteristics
	if (chgcol) {
		// obtain column (header) characteristics
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcFilList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcFilList", "ncol"));

		var TcoFnmWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcFilList", "TcoFnmWidth"));
		var TcoRetWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcFilList", "TcoRetWidth"));
		var TcoReuWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcFilList", "TcoReuWidth"));
		var TcoCntWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcFilList", "TcoCntWidth"));
		var TcoMimWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcFilList", "TcoMimWidth"));
		var TcoSizWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcFilList", "TcoSizWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacMsdcFilList", "numFTos"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagMsdcFilList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryMsdcFilList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListMsdcQFilList
		rownode = getRownode(srcdoc, "ListMsdcQFilList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagMsdcFilList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagMsdcFilList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfMsdcFilList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcFilList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcFilList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcFilList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryMsdcFilList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryMsdcFilList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppMsdcFilListData", scrJref, "StgIacQryMsdcFilList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcFilList", "firstcol"));

	setSi(srcdoc, "StatAppQryMsdcFilList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcFilList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppMsdcFilListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacMsdcFilList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacMsdcFilList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppMsdcFilListData", scrJref, "ContIacMsdcFilList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngMsdcFilListData", "ContIacMsdcFilList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngMsdcFilListData", "ContInfMsdcFilList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngMsdcFilListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngMsdcFilListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngMsdcFilListData", "StatShrMsdcFilList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngMsdcFilListData", "StgIacMsdcFilList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngMsdcFilListData", "TagMsdcFilList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngMsdcFilListData", "ListMsdcQFilList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngMsdcFilListData", "StatAppQryMsdcFilList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngMsdcFilListData", "StatShrQryMsdcFilList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngMsdcFilListData", "StgIacQryMsdcFilList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngMsdcFilListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngMsdcFilListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngMsdcFilListData") {
				mask = mergeDpchEngData(dom);
