...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppMsdcFil", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppMsdcFil", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppMsdcFil", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppMsdcFil", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppMsdcFil", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlMsdcFilHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlMsdcFilList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlMsdcFilRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppMsdcFil", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppMsdcFil", "shortMenu", short);
	setSi(srcdoc, "StatAppMsdcFil", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppMsdcFil", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppMsdcFil", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagMsdcFil", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagMsdcFil", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	MitCrdDldAvail = (retrieveSi(srcdoc, "StatShrMsdcFil", "MitCrdDldAvail") == "true");
	MitCrdDldActive = (retrieveSi(srcdoc, "StatShrMsdcFil", "MitCrdDldActive") == "true");

	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppMsdcFil", "widthMenu"));

	setTextContent(mendoc, mendoc.getElementById("MtxCrdFil"), retrieveCi(srcdoc, "ContInfMsdcFil", "MtxCrdFil"));
	setTextContent(mendoc, mendoc.getElementById("MitCrdDld"), retrieveTi(srcdoc, "TagMsdcFil", "MitCrdDld"));
...
function refresh() {
	var scrJrefDlgdownload = retrieveSi(srcdoc, "StatShrMsdcFil", "scrJrefDlgdownload");
	...
	doc.title = retrieveCi(srcdoc, "ContInfMsdcFil", "MtxCrdFil") + " - MultispectralDetectorControl 0.1.45";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppMsdcFilDo", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppMsdcFilDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfMsdcFil", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngMsdcFilData", "ContInfMsdcFil", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngMsdcFilData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngMsdcFilData", "StatAppMsdcFil", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngMsdcFilData", "StatShrMsdcFil", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngMsdcFilData", "TagMsdcFil", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngMsdcFilData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrMsdcFil", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrMsdcFil", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrMsdcFil", "scrJrefRec");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngMsdcFilData") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxMsdcVReqitmode = retrieveSi(srcdoc, "StatAppMsdcFil", "srefIxMsdcVReqitmode");

	if (srefIxMsdcVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxMsdcVReqitmode = "notify";
		setSi(srcdoc, "StatAppMsdcFil", "srefIxMsdcVReqitmode", srefIxMsdcVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxMsdcVReqitmode = retrieveSi(srcdoc, "StatAppMsdcFil", "srefIxMsdcVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppMsdcFil", "latency"));
