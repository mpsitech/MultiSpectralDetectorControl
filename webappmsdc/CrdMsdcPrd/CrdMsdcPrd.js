/**
  * \file CrdMsdcPrd.js
  * web client functionality for card CrdMsdcPrd
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppMsdcPrd", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppMsdcPrd", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppMsdcPrd", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppMsdcPrd", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppMsdcPrd", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlMsdcPrdHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlMsdcPrdList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlMsdcPrdRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppMsdcPrd", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppMsdcPrd", "shortMenu", short);
	setSi(srcdoc, "StatAppMsdcPrd", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppMsdcPrd", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppMsdcPrd", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagMsdcPrd", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagMsdcPrd", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	setTextContent(mendoc, mendoc.getElementById("MtxCrdPrd"), retrieveCi(srcdoc, "ContInfMsdcPrd", "MtxCrdPrd"));
};
...
function refresh() {
	...
	doc.title = retrieveCi(srcdoc, "ContInfMsdcPrd", "MtxCrdPrd") + " - MultispectralDetectorControl 0.1.52";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppMsdcPrdDo", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppMsdcPrdDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfMsdcPrd", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngMsdcPrdData", "ContInfMsdcPrd", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngMsdcPrdData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngMsdcPrdData", "StatAppMsdcPrd", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngMsdcPrdData", "StatShrMsdcPrd", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngMsdcPrdData", "TagMsdcPrd", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngMsdcPrdData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrMsdcPrd", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrMsdcPrd", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrMsdcPrd", "scrJrefRec");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngMsdcPrdData") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxMsdcVReqitmode = retrieveSi(srcdoc, "StatAppMsdcPrd", "srefIxMsdcVReqitmode");

	if (srefIxMsdcVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxMsdcVReqitmode = "notify";
		setSi(srcdoc, "StatAppMsdcPrd", "srefIxMsdcVReqitmode", srefIxMsdcVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxMsdcVReqitmode = retrieveSi(srcdoc, "StatAppMsdcPrd", "srefIxMsdcVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppMsdcPrd", "latency"));
