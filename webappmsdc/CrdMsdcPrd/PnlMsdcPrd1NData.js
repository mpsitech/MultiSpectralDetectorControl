/**
  * \file PnlMsdcPrd1NData.js
  * web client functionality for panel PnlMsdcPrd1NData
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppMsdcPrd1NData", "srefIxMsdcVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppMsdcPrd1NData", "srefIxMsdcVExpstate", "mind");

	getRecwnd().changeHeight("1NData", 30, updh);
	doc.getElementById("tdSide").setAttribute("height", "30");
	doc.getElementById("1NData_side").setAttribute("height", "30");
	doc.getElementById("1NData_cont").setAttribute("height", "30");

	// change content
	doc.getElementById("1NData_side").src = "./PnlMsdcPrd1NData_aside.html";
	doc.getElementById("1NData_cont").src = "./PnlMsdcPrd1NData_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppMsdcPrd1NData", "srefIxMsdcVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppMsdcPrd1NData", "srefIxMsdcVExpstate", "regd");

	getRecwnd().changeHeight("1NData", 255, true);
	doc.getElementById("tdSide").setAttribute("height", "255");
	doc.getElementById("1NData_side").setAttribute("height", "255");
	doc.getElementById("1NData_cont").setAttribute("height", "255");

	// change content
	doc.getElementById("1NData_side").src = "./PnlMsdcPrd1NData_bside.html";
	doc.getElementById("1NData_cont").src = "./PnlMsdcPrd1NData_b.html";
};
...
function initA() {
	...
	sidedoc = doc.getElementById("1NData_side").contentDocument;
	contdoc = doc.getElementById("1NData_cont").contentDocument;
	hdrdoc = null;
	tbldoc = null;
	ftrdoc = null;
...
function initBD(bNotD) {
	...
	sidedoc = doc.getElementById("1NData_side").contentDocument;
	contdoc = doc.getElementById("1NData_cont").contentDocument;
	hdrdoc = contdoc.getElementById("1NData_hdr").contentDocument;
	tbldoc = contdoc.getElementById("1NData_tbl").contentDocument;
	ftrdoc = contdoc.getElementById("1NData_ftr").contentDocument;

	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcPrd1NData", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagMsdcPrd1NData", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxMsdcVExpstate = retrieveSi(srcdoc, "StatAppMsdcPrd1NData", "srefIxMsdcVExpstate");

	if (srefIxMsdcVExpstate == "mind") initA();
	else if (srefIxMsdcVExpstate == "regd") initBD(true);
	else if (srefIxMsdcVExpstate == "detd") initBD(false);
};

function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcPrd1NData", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryMsdcPrd1NData", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcPrd1NData", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcPrd1NData", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcPrd1NData", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcPrd1NData", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcPrd1NData", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcPrd1NData", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcPrd1NData", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcPrd1NData", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcPrd1NData", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagMsdcPrd1NData", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryMsdcPrd1NData", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListMsdcQPrd1NData
		rownode = getRownode(srcdoc, "ListMsdcQPrd1NData", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagMsdcPrd1NData", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagMsdcPrd1NData", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfMsdcPrd1NData", "numFCsiQst"));

	height = 20 + 18*ndisp;

	getRecwnd().changeHeight("1NData", height+55, true);
	doc.getElementById("tdSide").setAttribute("height", "" + (height+55));
	doc.getElementById("1NData_side").setAttribute("height", "" + (height+55));
	doc.getElementById("1NData_cont").setAttribute("height", "" + (height+55));
	sidedoc.getElementById("tdFlex").setAttribute("height", "" + (height+55-30));
	contdoc.getElementById("tdTbl").setAttribute("height", "" + height);
	contdoc.getElementById("1NData_tbl").setAttribute("height", "" + height);
};
...
function refresh() {
	...
	else if (srefIxMsdcVExpstate == "regd") refreshBD(true, false);
	else if (srefIxMsdcVExpstate == "detd") refreshBD(false, false);
};
...
function handleLoad() {
	...
	doc = document;
	srcdoc = doc.getElementById("1NData_src").contentDocument;
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcPrd1NData", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcPrd1NData", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcPrd1NData", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryMsdcPrd1NData", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryMsdcPrd1NData", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppMsdcPrd1NDataData", scrJref, "StgIacQryMsdcPrd1NData");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppMsdcPrd1NData", "srefIxMsdcVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcPrd1NData", "firstcol"));

	setSi(srcdoc, "StatAppQryMsdcPrd1NData", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppMsdcPrd1NData", "srefIxMsdcVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcPrd1NData", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppMsdcPrd1NDataDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngMsdcPrd1NDataData", "ContInfMsdcPrd1NData", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngMsdcPrd1NDataData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngMsdcPrd1NDataData", "StatAppMsdcPrd1NData", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngMsdcPrd1NDataData", "StatShrMsdcPrd1NData", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngMsdcPrd1NDataData", "StgIacMsdcPrd1NData", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngMsdcPrd1NDataData", "TagMsdcPrd1NData", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngMsdcPrd1NDataData", "ListMsdcQPrd1NData", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngMsdcPrd1NDataData", "StatAppQryMsdcPrd1NData", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngMsdcPrd1NDataData", "StatShrQryMsdcPrd1NData", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngMsdcPrd1NDataData", "StgIacQryMsdcPrd1NData", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngMsdcPrd1NDataData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngMsdcPrd1NDataData") {
				mergeDpchEngData(dom);
				init();

				getRecwnd().setInitdone("1NData");
			};
		};
	};
};
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngMsdcPrd1NDataData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
