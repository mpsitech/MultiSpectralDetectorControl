/**
  * \file PnlMsdcPrdList.js
  * web client functionality for panel PnlMsdcPrdList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlMsdcPrdList_aside.html";
	doc.getElementById("List_cont").src = "./PnlMsdcPrdList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlMsdcPrdList_bside.html";
	doc.getElementById("List_cont").src = "./PnlMsdcPrdList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcPrdList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagMsdcPrdList", "Trs"));
...
function init() {
	var srefIxMsdcVExpstate = retrieveSi(srcdoc, "StatShrMsdcPrdList", "srefIxMsdcVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcPrdList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryMsdcPrdList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcPrdList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcPrdList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcPrdList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcPrdList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcPrdList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcPrdList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcPrdList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcPrdList", "ncol"));

		var TcoStaWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcPrdList", "TcoStaWidth"));
		var TcoStoWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcPrdList", "TcoStoWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacMsdcPrdList", "numFTos"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagMsdcPrdList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryMsdcPrdList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListMsdcQPrdList
		rownode = getRownode(srcdoc, "ListMsdcQPrdList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagMsdcPrdList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagMsdcPrdList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfMsdcPrdList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcPrdList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcPrdList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcPrdList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryMsdcPrdList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryMsdcPrdList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppMsdcPrdListData", scrJref, "StgIacQryMsdcPrdList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcPrdList", "firstcol"));

	setSi(srcdoc, "StatAppQryMsdcPrdList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcPrdList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppMsdcPrdListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacMsdcPrdList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacMsdcPrdList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppMsdcPrdListData", scrJref, "ContIacMsdcPrdList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngMsdcPrdListData", "ContIacMsdcPrdList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngMsdcPrdListData", "ContInfMsdcPrdList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngMsdcPrdListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngMsdcPrdListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngMsdcPrdListData", "StatShrMsdcPrdList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngMsdcPrdListData", "StgIacMsdcPrdList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngMsdcPrdListData", "TagMsdcPrdList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngMsdcPrdListData", "ListMsdcQPrdList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngMsdcPrdListData", "StatAppQryMsdcPrdList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngMsdcPrdListData", "StatShrQryMsdcPrdList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngMsdcPrdListData", "StgIacQryMsdcPrdList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngMsdcPrdListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngMsdcPrdListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngMsdcPrdListData") {
				mask = mergeDpchEngData(dom);
