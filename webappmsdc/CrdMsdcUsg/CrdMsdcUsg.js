/**
  * \file CrdMsdcUsg.js
  * web client functionality for card CrdMsdcUsg
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppMsdcUsg", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppMsdcUsg", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppMsdcUsg", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppMsdcUsg", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppMsdcUsg", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlMsdcUsgHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlMsdcUsgList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlMsdcUsgRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppMsdcUsg", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppMsdcUsg", "shortMenu", short);
	setSi(srcdoc, "StatAppMsdcUsg", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppMsdcUsg", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppMsdcUsg", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagMsdcUsg", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagMsdcUsg", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	setTextContent(mendoc, mendoc.getElementById("MtxCrdUsg"), retrieveCi(srcdoc, "ContInfMsdcUsg", "MtxCrdUsg"));
};
...
function refresh() {
	...
	doc.title = retrieveCi(srcdoc, "ContInfMsdcUsg", "MtxCrdUsg") + " - MultispectralDetectorControl 0.1.45";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppMsdcUsgDo", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppMsdcUsgDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfMsdcUsg", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngMsdcUsgData", "ContInfMsdcUsg", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngMsdcUsgData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngMsdcUsgData", "StatAppMsdcUsg", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngMsdcUsgData", "StatShrMsdcUsg", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngMsdcUsgData", "TagMsdcUsg", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngMsdcUsgData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrMsdcUsg", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrMsdcUsg", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrMsdcUsg", "scrJrefRec");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngMsdcUsgData") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxMsdcVReqitmode = retrieveSi(srcdoc, "StatAppMsdcUsg", "srefIxMsdcVReqitmode");

	if (srefIxMsdcVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxMsdcVReqitmode = "notify";
		setSi(srcdoc, "StatAppMsdcUsg", "srefIxMsdcVReqitmode", srefIxMsdcVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxMsdcVReqitmode = retrieveSi(srcdoc, "StatAppMsdcUsg", "srefIxMsdcVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppMsdcUsg", "latency"));
