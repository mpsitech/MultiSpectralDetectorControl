/**
  * \file PnlMsdcUsgAAccess.js
  * web client functionality for panel PnlMsdcUsgAAccess
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppMsdcUsgAAccess", "srefIxMsdcVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppMsdcUsgAAccess", "srefIxMsdcVExpstate", "mind");
	...
	// change content
	doc.getElementById("AAccess_side").src = "./PnlMsdcUsgAAccess_aside.html";
	doc.getElementById("AAccess_cont").src = "./PnlMsdcUsgAAccess_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppMsdcUsgAAccess", "srefIxMsdcVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppMsdcUsgAAccess", "srefIxMsdcVExpstate", "regd");
	...
	// change content
	doc.getElementById("AAccess_side").src = "./PnlMsdcUsgAAccess_bside.html";
	doc.getElementById("AAccess_cont").src = "./PnlMsdcUsgAAccess_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcUsgAAccess", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagMsdcUsgAAccess", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxMsdcVExpstate = retrieveSi(srcdoc, "StatAppMsdcUsgAAccess", "srefIxMsdcVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcUsgAAccess", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryMsdcUsgAAccess", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcUsgAAccess", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsgAAccess", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsgAAccess", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcUsgAAccess", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcUsgAAccess", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcUsgAAccess", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsgAAccess", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsgAAccess", "ncol"));

		var TcoFegWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcUsgAAccess", "TcoFegWidth"));
		var TcoFeaWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcUsgAAccess", "TcoFeaWidth"));
		var TcoAccWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcUsgAAccess", "TcoAccWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagMsdcUsgAAccess", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryMsdcUsgAAccess", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListMsdcQUsgAAccess
		rownode = getRownode(srcdoc, "ListMsdcQUsgAAccess", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagMsdcUsgAAccess", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagMsdcUsgAAccess", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfMsdcUsgAAccess", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcUsgAAccess", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsgAAccess", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcUsgAAccess", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryMsdcUsgAAccess", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryMsdcUsgAAccess", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppMsdcUsgAAccessData", scrJref, "StgIacQryMsdcUsgAAccess");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppMsdcUsgAAccess", "srefIxMsdcVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsgAAccess", "firstcol"));

	setSi(srcdoc, "StatAppQryMsdcUsgAAccess", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppMsdcUsgAAccess", "srefIxMsdcVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcUsgAAccess", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppMsdcUsgAAccessDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngMsdcUsgAAccessData", "ContInfMsdcUsgAAccess", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngMsdcUsgAAccessData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngMsdcUsgAAccessData", "StatAppMsdcUsgAAccess", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngMsdcUsgAAccessData", "StatShrMsdcUsgAAccess", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngMsdcUsgAAccessData", "StgIacMsdcUsgAAccess", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngMsdcUsgAAccessData", "TagMsdcUsgAAccess", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngMsdcUsgAAccessData", "ListMsdcQUsgAAccess", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngMsdcUsgAAccessData", "StatAppQryMsdcUsgAAccess", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngMsdcUsgAAccessData", "StatShrQryMsdcUsgAAccess", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngMsdcUsgAAccessData", "StgIacQryMsdcUsgAAccess", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngMsdcUsgAAccessData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngMsdcUsgAAccessData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngMsdcUsgAAccessData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
