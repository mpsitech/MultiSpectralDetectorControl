/**
  * \file PnlMsdcUsgList.js
  * web client functionality for panel PnlMsdcUsgList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlMsdcUsgList_aside.html";
	doc.getElementById("List_cont").src = "./PnlMsdcUsgList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlMsdcUsgList_bside.html";
	doc.getElementById("List_cont").src = "./PnlMsdcUsgList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcUsgList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagMsdcUsgList", "Trs"));
...
function init() {
	var srefIxMsdcVExpstate = retrieveSi(srcdoc, "StatShrMsdcUsgList", "srefIxMsdcVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcUsgList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryMsdcUsgList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcUsgList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsgList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsgList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcUsgList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcUsgList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcUsgList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsgList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsgList", "ncol"));

		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcUsgList", "TcoSrfWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacMsdcUsgList", "numFTos"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagMsdcUsgList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryMsdcUsgList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListMsdcQUsgList
		rownode = getRownode(srcdoc, "ListMsdcQUsgList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagMsdcUsgList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagMsdcUsgList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfMsdcUsgList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcUsgList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsgList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcUsgList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryMsdcUsgList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryMsdcUsgList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppMsdcUsgListData", scrJref, "StgIacQryMsdcUsgList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsgList", "firstcol"));

	setSi(srcdoc, "StatAppQryMsdcUsgList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcUsgList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppMsdcUsgListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacMsdcUsgList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacMsdcUsgList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppMsdcUsgListData", scrJref, "ContIacMsdcUsgList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngMsdcUsgListData", "ContIacMsdcUsgList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngMsdcUsgListData", "ContInfMsdcUsgList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngMsdcUsgListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngMsdcUsgListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngMsdcUsgListData", "StatShrMsdcUsgList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngMsdcUsgListData", "StgIacMsdcUsgList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngMsdcUsgListData", "TagMsdcUsgList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngMsdcUsgListData", "ListMsdcQUsgList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngMsdcUsgListData", "StatAppQryMsdcUsgList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngMsdcUsgListData", "StatShrQryMsdcUsgList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngMsdcUsgListData", "StgIacQryMsdcUsgList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngMsdcUsgListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngMsdcUsgListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngMsdcUsgListData") {
				mask = mergeDpchEngData(dom);
