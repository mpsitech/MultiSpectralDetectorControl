/**
  * \file PnlMsdcUsgMNUser.js
  * web client functionality for panel PnlMsdcUsgMNUser
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppMsdcUsgMNUser", "srefIxMsdcVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppMsdcUsgMNUser", "srefIxMsdcVExpstate", "mind");
	...
	// change content
	doc.getElementById("MNUser_side").src = "./PnlMsdcUsgMNUser_aside.html";
	doc.getElementById("MNUser_cont").src = "./PnlMsdcUsgMNUser_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppMsdcUsgMNUser", "srefIxMsdcVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppMsdcUsgMNUser", "srefIxMsdcVExpstate", "regd");
	...
	// change content
	doc.getElementById("MNUser_side").src = "./PnlMsdcUsgMNUser_bside.html";
	doc.getElementById("MNUser_cont").src = "./PnlMsdcUsgMNUser_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcUsgMNUser", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagMsdcUsgMNUser", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxMsdcVExpstate = retrieveSi(srcdoc, "StatAppMsdcUsgMNUser", "srefIxMsdcVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcUsgMNUser", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryMsdcUsgMNUser", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcUsgMNUser", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsgMNUser", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsgMNUser", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcUsgMNUser", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcUsgMNUser", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcUsgMNUser", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsgMNUser", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsgMNUser", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcUsgMNUser", "TcoMrefWidth"));
		var TcoUlvWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcUsgMNUser", "TcoUlvWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagMsdcUsgMNUser", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryMsdcUsgMNUser", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListMsdcQUsgMNUser
		rownode = getRownode(srcdoc, "ListMsdcQUsgMNUser", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagMsdcUsgMNUser", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagMsdcUsgMNUser", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfMsdcUsgMNUser", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcUsgMNUser", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsgMNUser", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcUsgMNUser", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryMsdcUsgMNUser", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryMsdcUsgMNUser", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppMsdcUsgMNUserData", scrJref, "StgIacQryMsdcUsgMNUser");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppMsdcUsgMNUser", "srefIxMsdcVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsgMNUser", "firstcol"));

	setSi(srcdoc, "StatAppQryMsdcUsgMNUser", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppMsdcUsgMNUser", "srefIxMsdcVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcUsgMNUser", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppMsdcUsgMNUserDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngMsdcUsgMNUserData", "ContInfMsdcUsgMNUser", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngMsdcUsgMNUserData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngMsdcUsgMNUserData", "StatAppMsdcUsgMNUser", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngMsdcUsgMNUserData", "StatShrMsdcUsgMNUser", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngMsdcUsgMNUserData", "StgIacMsdcUsgMNUser", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngMsdcUsgMNUserData", "TagMsdcUsgMNUser", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngMsdcUsgMNUserData", "ListMsdcQUsgMNUser", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngMsdcUsgMNUserData", "StatAppQryMsdcUsgMNUser", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngMsdcUsgMNUserData", "StatShrQryMsdcUsgMNUser", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngMsdcUsgMNUserData", "StgIacQryMsdcUsgMNUser", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngMsdcUsgMNUserData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngMsdcUsgMNUserData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngMsdcUsgMNUserData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
