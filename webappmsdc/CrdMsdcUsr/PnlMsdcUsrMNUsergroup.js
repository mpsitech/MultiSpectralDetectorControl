/**
  * \file PnlMsdcUsrMNUsergroup.js
  * web client functionality for panel PnlMsdcUsrMNUsergroup
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppMsdcUsrMNUsergroup", "srefIxMsdcVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppMsdcUsrMNUsergroup", "srefIxMsdcVExpstate", "mind");
	...
	// change content
	doc.getElementById("MNUsergroup_side").src = "./PnlMsdcUsrMNUsergroup_aside.html";
	doc.getElementById("MNUsergroup_cont").src = "./PnlMsdcUsrMNUsergroup_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppMsdcUsrMNUsergroup", "srefIxMsdcVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppMsdcUsrMNUsergroup", "srefIxMsdcVExpstate", "regd");
	...
	// change content
	doc.getElementById("MNUsergroup_side").src = "./PnlMsdcUsrMNUsergroup_bside.html";
	doc.getElementById("MNUsergroup_cont").src = "./PnlMsdcUsrMNUsergroup_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcUsrMNUsergroup", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagMsdcUsrMNUsergroup", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxMsdcVExpstate = retrieveSi(srcdoc, "StatAppMsdcUsrMNUsergroup", "srefIxMsdcVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcUsrMNUsergroup", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryMsdcUsrMNUsergroup", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcUsrMNUsergroup", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsrMNUsergroup", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsrMNUsergroup", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcUsrMNUsergroup", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcUsrMNUsergroup", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcUsrMNUsergroup", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsrMNUsergroup", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsrMNUsergroup", "ncol"));

		var TcoMrefWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcUsrMNUsergroup", "TcoMrefWidth"));
		var TcoUlvWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcUsrMNUsergroup", "TcoUlvWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagMsdcUsrMNUsergroup", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryMsdcUsrMNUsergroup", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListMsdcQUsrMNUsergroup
		rownode = getRownode(srcdoc, "ListMsdcQUsrMNUsergroup", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagMsdcUsrMNUsergroup", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagMsdcUsrMNUsergroup", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfMsdcUsrMNUsergroup", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcUsrMNUsergroup", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsrMNUsergroup", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcUsrMNUsergroup", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryMsdcUsrMNUsergroup", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryMsdcUsrMNUsergroup", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppMsdcUsrMNUsergroupData", scrJref, "StgIacQryMsdcUsrMNUsergroup");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppMsdcUsrMNUsergroup", "srefIxMsdcVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsrMNUsergroup", "firstcol"));

	setSi(srcdoc, "StatAppQryMsdcUsrMNUsergroup", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppMsdcUsrMNUsergroup", "srefIxMsdcVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcUsrMNUsergroup", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppMsdcUsrMNUsergroupDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngMsdcUsrMNUsergroupData", "ContInfMsdcUsrMNUsergroup", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngMsdcUsrMNUsergroupData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngMsdcUsrMNUsergroupData", "StatAppMsdcUsrMNUsergroup", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngMsdcUsrMNUsergroupData", "StatShrMsdcUsrMNUsergroup", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngMsdcUsrMNUsergroupData", "StgIacMsdcUsrMNUsergroup", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngMsdcUsrMNUsergroupData", "TagMsdcUsrMNUsergroup", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngMsdcUsrMNUsergroupData", "ListMsdcQUsrMNUsergroup", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngMsdcUsrMNUsergroupData", "StatAppQryMsdcUsrMNUsergroup", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngMsdcUsrMNUsergroupData", "StatShrQryMsdcUsrMNUsergroup", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngMsdcUsrMNUsergroupData", "StgIacQryMsdcUsrMNUsergroup", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngMsdcUsrMNUsergroupData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngMsdcUsrMNUsergroupData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngMsdcUsrMNUsergroupData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
