/**
  * \file PnlMsdcUsr1NSession.js
  * web client functionality for panel PnlMsdcUsr1NSession
...
function minimize(updh) {
	if (retrieveSi(srcdoc, "StatAppMsdcUsr1NSession", "srefIxMsdcVExpstate") == "mind") return;

	setSi(srcdoc, "StatAppMsdcUsr1NSession", "srefIxMsdcVExpstate", "mind");
	...
	// change content
	doc.getElementById("1NSession_side").src = "./PnlMsdcUsr1NSession_aside.html";
	doc.getElementById("1NSession_cont").src = "./PnlMsdcUsr1NSession_a.html";
};

function regularize() {
	if (retrieveSi(srcdoc, "StatAppMsdcUsr1NSession", "srefIxMsdcVExpstate") == "regd") return;

	setSi(srcdoc, "StatAppMsdcUsr1NSession", "srefIxMsdcVExpstate", "regd");
	...
	// change content
	doc.getElementById("1NSession_side").src = "./PnlMsdcUsr1NSession_bside.html";
	doc.getElementById("1NSession_cont").src = "./PnlMsdcUsr1NSession_b.html";
};
...
function initBD(bNotD) {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcUsr1NSession", "Cpt"));

	// column headers in refreshBD() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagMsdcUsr1NSession", "Trs"));

	refreshBD(bNotD, true);
};

function init() {
	var srefIxMsdcVExpstate = retrieveSi(srcdoc, "StatAppMsdcUsr1NSession", "srefIxMsdcVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcUsr1NSession", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryMsdcUsr1NSession", "ntot") + ")");
};

function refreshBD(bNotD, chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcUsr1NSession", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsr1NSession", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsr1NSession", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcUsr1NSession", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcUsr1NSession", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcUsr1NSession", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsr1NSession", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsr1NSession", "ncol"));

		var TcoRefWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcUsr1NSession", "TcoRefWidth"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			mytd.setAttribute("class", "col");
			setTextContent(tbldoc, mytd, retrieveTi(srcdoc, "TagMsdcUsr1NSession", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryMsdcUsr1NSession", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListMsdcQUsr1NSession
		rownode = getRownode(srcdoc, "ListMsdcQUsr1NSession", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagMsdcUsr1NSession", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagMsdcUsr1NSession", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfMsdcUsr1NSession", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcUsr1NSession", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsr1NSession", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcUsr1NSession", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryMsdcUsr1NSession", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryMsdcUsr1NSession", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppMsdcUsr1NSessionData", scrJref, "StgIacQryMsdcUsr1NSession");
...
function changeTblJnumFirstdisp(jnumFirstdisp, dJnumFirstdisp, dpgJnumFirstdisp) {
	...
	if (oldJnumFirstdisp != jnumFirstdisp) {
		...
		if ((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1))) {
			refreshBD((retrieveSi(srcdoc, "StatAppMsdcUsr1NSession", "srefIxMsdcVExpstate") == "regd"), false);
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsr1NSession", "firstcol"));

	setSi(srcdoc, "StatAppQryMsdcUsr1NSession", "firstcol", "" + (oldFirstcol-1));

	refreshBD((retrieveSi(srcdoc, "StatAppMsdcUsr1NSession", "srefIxMsdcVExpstate") == "regd"), true);
};
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcUsr1NSession", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppMsdcUsr1NSessionDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngMsdcUsr1NSessionData", "ContInfMsdcUsr1NSession", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngMsdcUsr1NSessionData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngMsdcUsr1NSessionData", "StatAppMsdcUsr1NSession", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngMsdcUsr1NSessionData", "StatShrMsdcUsr1NSession", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngMsdcUsr1NSessionData", "StgIacMsdcUsr1NSession", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngMsdcUsr1NSessionData", "TagMsdcUsr1NSession", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngMsdcUsr1NSessionData", "ListMsdcQUsr1NSession", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngMsdcUsr1NSessionData", "StatAppQryMsdcUsr1NSession", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngMsdcUsr1NSessionData", "StatShrQryMsdcUsr1NSession", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngMsdcUsr1NSessionData", "StgIacQryMsdcUsr1NSession", srcdoc)) mask.push("stgiacqry");

	return mask;
};

function handleDpchEng(dom, dpch) {
	if (dpch == "DpchEngMsdcUsr1NSessionData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngMsdcUsr1NSessionData") {
				mergeDpchEngData(dom);
				init();
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngMsdcUsr1NSessionData") {
				mergeDpchEngData(dom);
				refresh();
			};
		};
