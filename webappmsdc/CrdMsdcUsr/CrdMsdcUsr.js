/**
  * \file CrdMsdcUsr.js
  * web client functionality for card CrdMsdcUsr
...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppMsdcUsr", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppMsdcUsr", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppMsdcUsr", "initdoneHeadbar") == "true");
	var initdoneList = (retrieveSi(srcdoc, "StatAppMsdcUsr", "initdoneList") == "true");
	var initdoneRec = (retrieveSi(srcdoc, "StatAppMsdcUsr", "initdoneRec") == "true");

	if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlMsdcUsrHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else if (!initdoneList) {
		doc.getElementById("List").src = "./PnlMsdcUsrList.html?scrJref=" + scrJrefList;
	} else if (!initdoneRec) {
		doc.getElementById("Rec").src = "./PnlMsdcUsrRec.html?scrJref=" + scrJrefRec;
	} else {
		doc.initdone = true;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppMsdcUsr", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppMsdcUsr", "shortMenu", short);
	setSi(srcdoc, "StatAppMsdcUsr", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppMsdcUsr", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppMsdcUsr", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagMsdcUsr", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagMsdcUsr", "MrlAppHlp"));
};
...
function initMenCrd() {
	...
	setTextContent(mendoc, mendoc.getElementById("MtxCrdUsr"), retrieveCi(srcdoc, "ContInfMsdcUsr", "MtxCrdUsr"));
};
...
function refresh() {
	...
	doc.title = retrieveCi(srcdoc, "ContInfMsdcUsr", "MtxCrdUsr") + " - MultispectralDetectorControl 0.1.45";
};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppMsdcUsrDo", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppMsdcUsrDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfMsdcUsr", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngMsdcUsrData", "ContInfMsdcUsr", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngMsdcUsrData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngMsdcUsrData", "StatAppMsdcUsr", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngMsdcUsrData", "StatShrMsdcUsr", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngMsdcUsrData", "TagMsdcUsr", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngMsdcUsrData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrMsdcUsr", "scrJrefHeadbar");
				scrJrefList = retrieveSi(srcdoc, "StatShrMsdcUsr", "scrJrefList");
				scrJrefRec = retrieveSi(srcdoc, "StatShrMsdcUsr", "scrJrefRec");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngMsdcUsrData") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxMsdcVReqitmode = retrieveSi(srcdoc, "StatAppMsdcUsr", "srefIxMsdcVReqitmode");

	if (srefIxMsdcVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxMsdcVReqitmode = "notify";
		setSi(srcdoc, "StatAppMsdcUsr", "srefIxMsdcVReqitmode", srefIxMsdcVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxMsdcVReqitmode = retrieveSi(srcdoc, "StatAppMsdcUsr", "srefIxMsdcVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppMsdcUsr", "latency"));
