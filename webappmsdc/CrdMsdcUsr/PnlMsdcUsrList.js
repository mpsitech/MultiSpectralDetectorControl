/**
  * \file PnlMsdcUsrList.js
  * web client functionality for panel PnlMsdcUsrList
...
function minimize() {
	...
	// change content
	doc.getElementById("List_side").src = "./PnlMsdcUsrList_aside.html";
	doc.getElementById("List_cont").src = "./PnlMsdcUsrList_a.html";
};

function regularize() {
	// change content (container heights in refreshB)
	doc.getElementById("List_side").src = "./PnlMsdcUsrList_bside.html";
	doc.getElementById("List_cont").src = "./PnlMsdcUsrList_b.html";
};
...
function initB() {
	...
	setTextContent(hdrdoc, hdrdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcUsrList", "Cpt"));

	// column headers in refreshB() only

	setTextContent(ftrdoc, ftrdoc.getElementById("TrsGoto"), retrieveTi(srcdoc, "TagMsdcUsrList", "Trs"));
...
function init() {
	var srefIxMsdcVExpstate = retrieveSi(srcdoc, "StatShrMsdcUsrList", "srefIxMsdcVExpstate");
...
function refreshA() {
	setTextContent(contdoc, contdoc.getElementById("Cpt"), retrieveTi(srcdoc, "TagMsdcUsrList", "Cpt") + " (" + retrieveSi(srcdoc, "StatShrQryMsdcUsrList", "ntot") + ")");
};

function refreshB(chgcol) {
	// obtain record set characteristics
	var jnumSel = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcUsrList", "jnum"));
	var jnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsrList", "jnumFirstdisp"));
	var ndisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsrList", "ndisp"));
	var jnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcUsrList", "jnumFirstload"));
	var nload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcUsrList", "nload"));
	var ntot = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcUsrList", "ntot"));
	...
	if (chgcol) {
		...
		var firstcol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsrList", "firstcol"));
		var ncol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsrList", "ncol"));

		var TcoPrsWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcUsrList", "TcoPrsWidth"));
		var TcoSrfWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcUsrList", "TcoSrfWidth"));
		var TcoUsgWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcUsrList", "TcoUsgWidth"));
		var TcoSteWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcUsrList", "TcoSteWidth"));
		var TcoLclWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcUsrList", "TcoLclWidth"));
		var TcoUlvWidth = parseInt(retrieveSi(srcdoc, "StgIacMsdcUsrList", "TcoUlvWidth"));

		var numFTos = parseInt(retrieveCi(srcdoc, "ContIacMsdcUsrList", "numFTos"));
		...
		for (i=0;i<doc.cols.length;i++) {
			...
			setTextContent(tbldoc, myspan, retrieveTi(srcdoc, "TagMsdcUsrList", doc.cols[i]));
	...
	if ((jnumFirstdisp < jnumFirstload) || (jnumFirstdisp > (jnumFirstload+nload-1))) {
		...
		setSi(srcdoc, "StatAppQryMsdcUsrList", "jnumFirstdisp", jnumFirstdisp);
	};
	...
	for (i=1;i<=ndisp;i++) {
		...
		// find element in ListMsdcQUsrList
		rownode = getRownode(srcdoc, "ListMsdcQUsrList", jnum);
	...
	setTextContent(ftrdoc, ftrdoc.getElementById("TxtShowing"), strShowing(retrieveTi(srcdoc, "TagMsdcUsrList", "TxtShowing1"), jnumFirstdisp, jnumLastdisp, ntot, retrieveTi(srcdoc, "TagMsdcUsrList", "TxtShowing2")));
	...
	refreshCsi(ftrdoc, "CsiQst", srcdoc, "FeedFCsiQst", retrieveCi(srcdoc, "ContInfMsdcUsrList", "numFCsiQst"));
...
function handleTblSelect(jnum) {
	...
	var oldJnum = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcUsrList", "jnum"));
	var oldJnumFirstdisp = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsrList", "jnumFirstdisp"));
	var oldJnumFirstload = parseInt(retrieveSi(srcdoc, "StatShrQryMsdcUsrList", "jnumFirstload"));
	...
	if (oldJnum != jnum) {
		setSi(srcdoc, "StgIacQryMsdcUsrList", "jnum", jnum);
		...
		if (jnum != 0) {
			if (!((jnum >= oldJnumFirstdisp) && (jnum <= (oldJnumFirstdisp+ndisp-1)))) {
				...
				if (!((jnumFirstdisp >= oldJnumFirstload) && (jnumLastdisp <= (oldJnumFirstload+nload-1)))) {
					...
					setSi(srcdoc, "StgIacQryMsdcUsrList", "jnumFirstload", jnumFirstload);
				};
			};
		};

		var str = serializeDpchAppData(srcdoc, "DpchAppMsdcUsrListData", scrJref, "StgIacQryMsdcUsrList");
...
function handleTblCpreviousClick() {
	var oldFirstcol = parseInt(retrieveSi(srcdoc, "StatAppQryMsdcUsrList", "firstcol"));

	setSi(srcdoc, "StatAppQryMsdcUsrList", "firstcol", "" + (oldFirstcol-1));
...
function handleTrsPreviousClick() {
	var jnum = parseInt(retrieveSi(srcdoc, "StgIacQryMsdcUsrList", "jnum"));
...
function handleButClick(ctlsref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppMsdcUsrListDo", scrJref, ctlsref + "Click");
	sendReq(str, doc, handleDpchAppDataDoReply);
};

function handleTosSelect(numFTos) {
	var oldNumFTos = parseInt(retrieveCi(srcdoc, "ContIacMsdcUsrList", "numFTos"));

	if (oldNumFTos != numFTos) {
		setCi(srcdoc, "ContIacMsdcUsrList", "numFTos", "" + numFTos);

		var str = serializeDpchAppData(srcdoc, "DpchAppMsdcUsrListData", scrJref, "ContIacMsdcUsrList");
		sendReq(str, doc, handleDpchAppDataDoReply);
	};
};
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngMsdcUsrListData", "ContIacMsdcUsrList", srcdoc)) mask.push("contiac");
	if (updateSrcblock(dom, "DpchEngMsdcUsrListData", "ContInfMsdcUsrList", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngMsdcUsrListData", "FeedFCsiQst", srcdoc)) mask.push("feedFCsiQst");
	if (updateSrcblock(dom, "DpchEngMsdcUsrListData", "FeedFTos", srcdoc)) mask.push("feedFTos");
	if (updateSrcblock(dom, "DpchEngMsdcUsrListData", "StatShrMsdcUsrList", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngMsdcUsrListData", "StgIacMsdcUsrList", srcdoc)) mask.push("stgiac");
	if (updateSrcblock(dom, "DpchEngMsdcUsrListData", "TagMsdcUsrList", srcdoc)) mask.push("tag");
	if (updateSrcblock(dom, "DpchEngMsdcUsrListData", "ListMsdcQUsrList", srcdoc)) mask.push("rst");
	if (updateSrcblock(dom, "DpchEngMsdcUsrListData", "StatAppQryMsdcUsrList", srcdoc)) mask.push("statappqry");
	if (updateSrcblock(dom, "DpchEngMsdcUsrListData", "StatShrQryMsdcUsrList", srcdoc)) mask.push("statshrqry");
	if (updateSrcblock(dom, "DpchEngMsdcUsrListData", "StgIacQryMsdcUsrList", srcdoc)) mask.push("stgiacqry");
...
function handleDpchEng(dom, dpch) {
	...
	if (dpch == "DpchEngMsdcUsrListData") {
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngMsdcUsrListData") {
...
function handleDpchAppDataDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngMsdcUsrListData") {
				mask = mergeDpchEngData(dom);
