...
function getInitdone(pnlshort) {
	return(retrieveSi(srcdoc, "StatAppMsdcLiv", "initdone" + pnlshort) == "true");
};

function setInitdone(pnlshort) {
	setSi(srcdoc, "StatAppMsdcLiv", "initdone" + pnlshort, "true");

	checkInitdone();
};

function checkInitdone() {
	var initdoneVideo = (retrieveSi(srcdoc, "StatAppMsdcLiv", "initdoneVideo") == "true");
	var initdoneAlign = (retrieveSi(srcdoc, "StatAppMsdcLiv", "initdoneAlign") == "true");
	var initdoneScill = (retrieveSi(srcdoc, "StatAppMsdcLiv", "initdoneScill") == "true");
	var initdoneTrack = (retrieveSi(srcdoc, "StatAppMsdcLiv", "initdoneTrack") == "true");
	var initdoneHeadbar = (retrieveSi(srcdoc, "StatAppMsdcLiv", "initdoneHeadbar") == "true");

	if (!initdoneVideo) {
		doc.getElementById("Video").src = "./PnlMsdcLivVideo.html?scrJref=" + scrJrefVideo;
	} else if (!initdoneAlign) {
		doc.getElementById("Align").src = "./PnlMsdcLivAlign.html?scrJref=" + scrJrefAlign;
	} else if (!initdoneScill) {
		doc.getElementById("Scill").src = "./PnlMsdcLivScill.html?scrJref=" + scrJrefScill;
	} else if (!initdoneTrack) {
		doc.getElementById("Track").src = "./PnlMsdcLivTrack.html?scrJref=" + scrJrefTrack;
	} else if (!initdoneHeadbar) {
		doc.getElementById("Headbar").src = "./PnlMsdcLivHeadbar.html?scrJref=" + scrJrefHeadbar;
	} else {
		doc.initdone = true;
...
function getHeight() {
	...
	height += 10 + parseInt(doc.getElementById("tdVideo").getAttribute("height")) + 8;
	height += 10 + parseInt(doc.getElementById("tdAlign").getAttribute("height")) + 8;
	height += 10 + parseInt(doc.getElementById("tdScill").getAttribute("height")) + 8;
	height += 10 + parseInt(doc.getElementById("tdTrack").getAttribute("height")) + 8;
...
function toggleMenu(short, left, width, height) {
	var shortMenu = retrieveSi(srcdoc, "StatAppMsdcLiv", "shortMenu");
...
function showMenu(short, left, width, height) {
	...
	setSi(srcdoc, "StatAppMsdcLiv", "shortMenu", short);
	setSi(srcdoc, "StatAppMsdcLiv", "widthMenu", "" + (width-3));
...
function hideMenu(short) {
	...
	setSi(srcdoc, "StatAppMsdcLiv", "shortMenu", "");
...
function initMenApp() {
	...
	mendoc.getElementById("colCont").setAttribute("width", retrieveSi(srcdoc, "StatAppMsdcLiv", "widthMenu"));
	setTextContent(mendoc, mendoc.getElementById("MitAppAbt"), retrieveTi(srcdoc, "TagMsdcLiv", "MitAppAbt"));
	setTextContent(mendoc, mendoc.getElementById("MrlAppHlp"), retrieveTi(srcdoc, "TagMsdcLiv", "MrlAppHlp"));
};
...
function changeHeight(pnlshort, height) {
	...
	if (pnlshort == "Video") {
		doc.getElementById("tdVideo").setAttribute("height", "" + height);
		doc.getElementById("Video").setAttribute("height", "" + height);
	} else if (pnlshort == "Align") {
		doc.getElementById("tdAlign").setAttribute("height", "" + height);
		doc.getElementById("Align").setAttribute("height", "" + height);
	...
	} else if (pnlshort == "Track") {
		doc.getElementById("tdTrack").setAttribute("height", "" + height);
		doc.getElementById("Track").setAttribute("height", "" + height);
	};
...
function handleUnload() {
	var str = serializeDpchAppDo(srcdoc, "DpchAppMsdcLivDo", scrJref, "close");
	sendReq(str, doc);
};

function handleMitClick(menshort, consref) {
	var str = serializeDpchAppDo(srcdoc, "DpchAppMsdcLivDo", scrJref, consref + "Click");
	sendReq(str, doc, handleDpchAppDoReply);
...
function handleMrlClick(menshort, consref) {
	window.open(retrieveCi(srcdoc, "ContInfMsdcLiv", consref), "_blank");
...
function mergeDpchEngData(dom) {
	...
	if (updateSrcblock(dom, "DpchEngMsdcLivData", "ContInfMsdcLiv", srcdoc)) mask.push("continf");
	if (updateSrcblock(dom, "DpchEngMsdcLivData", "FeedFSge", srcdoc)) mask.push("feedFSge");
	if (updateSrcblock(dom, "DpchEngMsdcLivData", "StatAppMsdcLiv", srcdoc)) mask.push("statapp");
	if (updateSrcblock(dom, "DpchEngMsdcLivData", "StatShrMsdcLiv", srcdoc)) mask.push("statshr");
	if (updateSrcblock(dom, "DpchEngMsdcLivData", "TagMsdcLiv", srcdoc)) mask.push("tag");
...
function handleDpchAppInitReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			if (blk.nodeName == "DpchEngMsdcLivData") {
				mergeDpchEngData(dom);

				// establish sub-panel scrJref's as global variables, and start sub-panel load sequence
				scrJrefVideo = retrieveSi(srcdoc, "StatShrMsdcLiv", "scrJrefVideo");
				scrJrefAlign = retrieveSi(srcdoc, "StatShrMsdcLiv", "scrJrefAlign");
				scrJrefScill = retrieveSi(srcdoc, "StatShrMsdcLiv", "scrJrefScill");
				scrJrefTrack = retrieveSi(srcdoc, "StatShrMsdcLiv", "scrJrefTrack");
				scrJrefHeadbar = retrieveSi(srcdoc, "StatShrMsdcLiv", "scrJrefHeadbar");
...
function handleDpchAppDoReply() {
	...
	if (doc.req.readyState == 4) {
		...
		if (blk) {
			...
			} else if (blk.nodeName == "DpchEngMsdcLivData") {
				mergeDpchEngData(dom);
				refresh();
			};
...
function restartNotify() {
	var srefIxMsdcVReqitmode = retrieveSi(srcdoc, "StatAppMsdcLiv", "srefIxMsdcVReqitmode");

	if (srefIxMsdcVReqitmode != "notify") {
		if (doc.timeout) window.clearTimeout(doc.timeout);

		srefIxMsdcVReqitmode = "notify";
		setSi(srcdoc, "StatAppMsdcLiv", "srefIxMsdcVReqitmode", srefIxMsdcVReqitmode);
...
function iterateReqit(ev) {
	...
	var srefIxMsdcVReqitmode = retrieveSi(srcdoc, "StatAppMsdcLiv", "srefIxMsdcVReqitmode");
	var latency = parseInt(retrieveSi(srcdoc, "StatAppMsdcLiv", "latency"));
			...
			if (blk) {
				if (blk.nodeName == "DpchEngMsdcAck") {
					...
					if (blk.nodeName == "DpchEngMsdcAlert") {
						...
						} else if (_scrJref == scrJrefVideo) {
							doc.getElementById("Video").contentWindow.handleDpchEng(dom, blk.nodeName);
						} else if (_scrJref == scrJrefAlign) {
							doc.getElementById("Align").contentWindow.handleDpchEng(dom, blk.nodeName);
						} else if (_scrJref == scrJrefScill) {
							doc.getElementById("Scill").contentWindow.handleDpchEng(dom, blk.nodeName);
						} else if (_scrJref == scrJrefTrack) {
							doc.getElementById("Track").contentWindow.handleDpchEng(dom, blk.nodeName);
