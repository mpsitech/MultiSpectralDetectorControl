/**
  * \file PnlMsdcUsrAAccess_blks.cpp
  * job handler for job PnlMsdcUsrAAccess (implementation of blocks)
...
void PnlMsdcUsrAAccess::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfMsdcUsrAAccess";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfMsdcUsrAAccess";
...
void PnlMsdcUsrAAccess::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixMsdcVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppMsdcUsrAAccess";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppMsdcUsrAAccess";
...
void PnlMsdcUsrAAccess::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrMsdcUsrAAccess";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrMsdcUsrAAccess";
...
void PnlMsdcUsrAAccess::Tag::writeXML(
			const uint ixMsdcVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagMsdcUsrAAccess";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemMsdcUsrAAccess";
...
PnlMsdcUsrAAccess::DpchAppData::DpchAppData() : DpchAppMsdc(VecMsdcVDpch::DPCHAPPMSDCUSRAACCESSDATA) {
};
...
void PnlMsdcUsrAAccess::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppMsdcUsrAAccessData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlMsdcUsrAAccess::DpchAppDo::DpchAppDo() : DpchAppMsdc(VecMsdcVDpch::DPCHAPPMSDCUSRAACCESSDO) {
	ixVDo = 0;
};
...
void PnlMsdcUsrAAccess::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppMsdcUsrAAccessDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlMsdcUsrAAccess::DpchEngData::DpchEngData(
			...
			, const set<uint>& mask
		) : DpchEngMsdc(VecMsdcVDpch::DPCHENGMSDCUSRAACCESSDATA, jref) {
...
void PnlMsdcUsrAAccess::DpchEngData::merge(
			DpchEngMsdc* dpcheng
		) {
...
void PnlMsdcUsrAAccess::DpchEngData::writeXML(
			const uint ixMsdcVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngMsdcUsrAAccessData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/msdc");
		...
		if (has(STATAPPQRY)) QryMsdcUsrAAccess::StatApp::writeXML(wr);
