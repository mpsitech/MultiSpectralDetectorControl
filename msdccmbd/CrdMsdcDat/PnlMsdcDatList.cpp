/**
  ...
  * job handler for job PnlMsdcDatList (implementation)
...
#include "PnlMsdcDatList_blks.cpp"
#include "PnlMsdcDatList_evals.cpp"

/******************************************************************************
 class PnlMsdcDatList
 ******************************************************************************/

PnlMsdcDatList::PnlMsdcDatList(
			XchgMsdc* xchg
			, DbsMsdc* dbsmsdc
			, const ubigint jrefSup
			, const uint ixMsdcVLocale
		) :
			JobMsdc(xchg, VecMsdcVJob::PNLMSDCDATLIST, jrefSup, ixMsdcVLocale)
		{
	...
	QryMsdcDatList::VecVOrd::fillFeed(feedFTos);

	qry = NULL;

	// IP constructor.cust1 --- INSERT

	xchg->addRefPreset(VecMsdcVPreset::PREMSDCREFSEL, jref, xchg->getRefPreset(VecMsdcVPreset::PREMSDCREFDAT, jref));
	xchg->addIxPreset(VecMsdcVPreset::PREMSDCIXORD, jref, QryMsdcDatList::VecVOrd::AFT);

	qry = new QryMsdcDatList(xchg, dbsmsdc, jref, ixMsdcVLocale);

	// IP constructor.cust2 --- INSERT

	set<uint> moditems;
	statshr.ixMsdcVExpstate = ((xchg->getRefPreset(VecMsdcVPreset::PREMSDCREFDAT, jref)) ? VecMsdcVExpstate::MIND : VecMsdcVExpstate::REGD);
...
// IP cust --- INSERT
DpchEngMsdc* PnlMsdcDatList::getNewDpchEng(
			set<uint> items
		) {
...
void PnlMsdcDatList::refresh(
	...
	if ((ixPre != 0) && (ixPre != VecMsdcVPreset::VOID)) {
		...
		if (ixPre == VecMsdcVPreset::PREMSDCREFPRD) continf.TxtPre = StubMsdc::getStubPrdStd(dbsmsdc, xchg->getRefPreset(ixPre, jref), ixMsdcVLocale, Stub::VecVNonetype::FULL);
...
void PnlMsdcDatList::updatePreset(
			DbsMsdc* dbsmsdc
...
void PnlMsdcDatList::regularize(
			DbsMsdc* dbsmsdc
			, const bool notif
			, DpchEngMsdc** dpcheng
		) {
...
void PnlMsdcDatList::handleRequest(
			DbsMsdc* dbsmsdc
			, ReqMsdc* req
		) {
	if (req->ixVBasetype == ReqMsdc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixMsdcVDpch == VecMsdcVDpch::DPCHAPPMSDCDATLISTDATA) {
		...
		} else if (req->dpchapp->ixMsdcVDpch == VecMsdcVDpch::DPCHAPPMSDCDATLISTDO) {
...
void PnlMsdcDatList::handleDpchAppMsdcInit(
			DbsMsdc* dbsmsdc
...
void PnlMsdcDatList::handleDpchAppDataContiac(
			DbsMsdc* dbsmsdc
			, ContIac* _contiac
			, DpchEngMsdc** dpcheng
		) {
	...
	if (has(diffitems, ContIac::NUMFTOS)) {
		if ((_contiac->numFTos >= QryMsdcDatList::VecVOrd::AFT) && (_contiac->numFTos <= QryMsdcDatList::VecVOrd::STA)) {
...
void PnlMsdcDatList::handleDpchAppDataStgiac(
			DbsMsdc* dbsmsdc
			, StgIac* _stgiac
			, DpchEngMsdc** dpcheng
		) {
...
void PnlMsdcDatList::handleDpchAppDataStgiacqry(
			DbsMsdc* dbsmsdc
			, QryMsdcDatList::StgIac* _stgiacqry
			, DpchEngMsdc** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryMsdcDatList::StgIac::JNUM)) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryMsdcDatList::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsmsdc);
			refresh(dbsmsdc, moditems);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryMsdcDatList::StgIac::JNUM)) {
			if (refSelNew == 0) refSelNew = qry->getRefByJnum(_stgiacqry->jnum);
			xchg->triggerIxRefCall(dbsmsdc, VecMsdcVCall::CALLMSDCREFPRESET, jref, VecMsdcVPreset::PREMSDCREFDAT, refSelNew);
		};
	};
...
void PnlMsdcDatList::handleDpchAppDoButMinimizeClick(
			DbsMsdc* dbsmsdc
			, DpchEngMsdc** dpcheng
		) {
...
void PnlMsdcDatList::handleDpchAppDoButRegularizeClick(
			DbsMsdc* dbsmsdc
			, DpchEngMsdc** dpcheng
		) {
...
void PnlMsdcDatList::handleDpchAppDoButNewClick(
			DbsMsdc* dbsmsdc
			, DpchEngMsdc** dpcheng
		) {
...
void PnlMsdcDatList::handleDpchAppDoButDeleteClick(
			DbsMsdc* dbsmsdc
			, DpchEngMsdc** dpcheng
		) {
...
void PnlMsdcDatList::handleDpchAppDoButFilterClick(
			DbsMsdc* dbsmsdc
			, DpchEngMsdc** dpcheng
		) {
...
void PnlMsdcDatList::handleDpchAppDoButRefreshClick(
			DbsMsdc* dbsmsdc
			, DpchEngMsdc** dpcheng
		) {
	set<uint> moditems;
...
void PnlMsdcDatList::handleCall(
			DbsMsdc* dbsmsdc
			, Call* call
		) {
...
bool PnlMsdcDatList::handleCallMsdcStatChg(
			DbsMsdc* dbsmsdc
			, const ubigint jrefTrig
		) {
	bool retval = false;
