...
void PnlMsdcFilDetail::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfMsdcFilDetail";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfMsdcFilDetail";
...
void PnlMsdcFilDetail::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	if (difftag.length() == 0) difftag = "StatAppMsdcFilDetail";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppMsdcFilDetail";
...
void PnlMsdcFilDetail::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrMsdcFilDetail";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrMsdcFilDetail";
...
void PnlMsdcFilDetail::Tag::writeXML(
			const uint ixMsdcVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagMsdcFilDetail";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemMsdcFilDetail";
	...
	xmlTextWriterStartElement(wr, BAD_CAST difftag.c_str());
		...
		writeStringAttr(wr, itemtag, "sref", "CptClu", VecMsdcVTag::getTitle(VecMsdcVTag::CLUST, ixMsdcVLocale));
	xmlTextWriterEndElement(wr);
};
...
void PnlMsdcFilDetail::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppMsdcFilDetailData");
	else
		basefound = checkXPath(docctx, basexpath);
...
void PnlMsdcFilDetail::DpchAppDo::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppMsdcFilDetailDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
void PnlMsdcFilDetail::DpchEngData::merge(
			DpchEngMsdc* dpcheng
		) {
