/**
  * \file JobMsdcAcqVisr_blks.cpp
  * job handler for job JobMsdcAcqVisr (implementation of blocks)
...
/******************************************************************************
 class JobMsdcAcqVisr::VecVMethod
 ******************************************************************************/

uint JobMsdcAcqVisr::VecVMethod::getIx(
			const string& sref
		) {
...
string JobMsdcAcqVisr::VecVMethod::getSref(
			const uint ix
		) {
...
void JobMsdcAcqVisr::VecVMethod::fillFeed(
			Feed& feed
		) {
	feed.clear();
...
/******************************************************************************
 class JobMsdcAcqVisr::VecVSge
 ******************************************************************************/

uint JobMsdcAcqVisr::VecVSge::getIx(
			const string& sref
		) {
	string s = StrMod::lc(sref);
...
string JobMsdcAcqVisr::VecVSge::getSref(
			const uint ix
		) {
...
void JobMsdcAcqVisr::VecVSge::fillFeed(
			Feed& feed
		) {
	feed.clear();
...
/******************************************************************************
 class JobMsdcAcqVisr::VecVVar
 ******************************************************************************/

uint JobMsdcAcqVisr::VecVVar::getIx(
			const string& sref
		) {
	string s = StrMod::lc(sref);
...
string JobMsdcAcqVisr::VecVVar::getSref(
			const uint ix
		) {
...
void JobMsdcAcqVisr::VecVVar::fillFeed(
			Feed& feed
		) {
	feed.clear();
...
/******************************************************************************
 class JobMsdcAcqVisr::Stg
 ******************************************************************************/

JobMsdcAcqVisr::Stg::Stg(
...
bool JobMsdcAcqVisr::Stg::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgJobMsdcAcqVisr");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemJobMsdcAcqVisr";
...
void JobMsdcAcqVisr::Stg::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StgJobMsdcAcqVisr";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StgitemJobMsdcAcqVisr";
...
set<uint> JobMsdcAcqVisr::Stg::comm(
			const Stg* comp
		) {
	set<uint> items;
...
set<uint> JobMsdcAcqVisr::Stg::diff(
			const Stg* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
	...
	diffitems = {IXMSDCVCAMTYPE, WIDTH, HEIGHT, HPIX, PATH, SERNO, IXMSDCVSQRGRP, F, FN, K1, K2, P1, P2, K3, K4, K5, K6};
