/**
  * \file PnlMsdcUsgRec_blks.cpp
  * job handler for job PnlMsdcUsgRec (implementation of blocks)
...
void PnlMsdcUsgRec::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfMsdcUsgRec";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfMsdcUsgRec";
...
void PnlMsdcUsgRec::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	if (difftag.length() == 0) difftag = "StatAppMsdcUsgRec";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppMsdcUsgRec";
...
PnlMsdcUsgRec::StatShr::StatShr(
	...
	mask = {IXMSDCVEXPSTATE, JREFDETAIL, JREFAACCESS, JREFMNUSER, BUTREGULARIZEACTIVE};
};

void PnlMsdcUsgRec::StatShr::writeXML(
	...
	if (difftag.length() == 0) difftag = "StatShrMsdcUsgRec";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrMsdcUsgRec";
...
set<uint> PnlMsdcUsgRec::StatShr::diff(
	...
	diffitems = {IXMSDCVEXPSTATE, JREFDETAIL, JREFAACCESS, JREFMNUSER, BUTREGULARIZEACTIVE};
	for (auto it=commitems.begin();it!=commitems.end();it++) diffitems.erase(*it);
...
void PnlMsdcUsgRec::Tag::writeXML(
			const uint ixMsdcVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagMsdcUsgRec";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemMsdcUsgRec";
...
PnlMsdcUsgRec::DpchAppDo::DpchAppDo() : DpchAppMsdc(VecMsdcVDpch::DPCHAPPMSDCUSGRECDO) {
	ixVDo = 0;
};
...
void PnlMsdcUsgRec::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppMsdcUsgRecDo");
	else
...
PnlMsdcUsgRec::DpchEngData::DpchEngData(
			...
			, StatShr* statshr
			, const set<uint>& mask
		) : DpchEngMsdc(VecMsdcVDpch::DPCHENGMSDCUSGRECDATA, jref) {
...
void PnlMsdcUsgRec::DpchEngData::merge(
			DpchEngMsdc* dpcheng
		) {
...
void PnlMsdcUsgRec::DpchEngData::writeXML(
			const uint ixMsdcVLocale
			, pthread_mutex_t* mScr
			, map<ubigint,string>& scr
			, map<string,ubigint>& descr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngMsdcUsgRecData");
