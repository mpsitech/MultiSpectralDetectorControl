/**
  * \file PnlMsdcUsgMNUser_blks.cpp
  * job handler for job PnlMsdcUsgMNUser (implementation of blocks)
...
void PnlMsdcUsgMNUser::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfMsdcUsgMNUser";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfMsdcUsgMNUser";
...
void PnlMsdcUsgMNUser::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixMsdcVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppMsdcUsgMNUser";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppMsdcUsgMNUser";
...
void PnlMsdcUsgMNUser::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrMsdcUsgMNUser";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrMsdcUsgMNUser";
...
void PnlMsdcUsgMNUser::Tag::writeXML(
			const uint ixMsdcVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagMsdcUsgMNUser";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemMsdcUsgMNUser";
...
PnlMsdcUsgMNUser::DpchAppData::DpchAppData() : DpchAppMsdc(VecMsdcVDpch::DPCHAPPMSDCUSGMNUSERDATA) {
};
...
void PnlMsdcUsgMNUser::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppMsdcUsgMNUserData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlMsdcUsgMNUser::DpchAppDo::DpchAppDo() : DpchAppMsdc(VecMsdcVDpch::DPCHAPPMSDCUSGMNUSERDO) {
	ixVDo = 0;
};
...
void PnlMsdcUsgMNUser::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppMsdcUsgMNUserDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlMsdcUsgMNUser::DpchEngData::DpchEngData(
			...
			, const set<uint>& mask
		) : DpchEngMsdc(VecMsdcVDpch::DPCHENGMSDCUSGMNUSERDATA, jref) {
...
void PnlMsdcUsgMNUser::DpchEngData::merge(
			DpchEngMsdc* dpcheng
		) {
...
void PnlMsdcUsgMNUser::DpchEngData::writeXML(
			const uint ixMsdcVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngMsdcUsgMNUserData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/msdc");
		...
		if (has(STATAPPQRY)) QryMsdcUsgMNUser::StatApp::writeXML(wr);
