/**
  ...
  * job handler for job PnlMsdcUsgMNUser (implementation)
...
#include "PnlMsdcUsgMNUser_blks.cpp"
#include "PnlMsdcUsgMNUser_evals.cpp"

/******************************************************************************
 class PnlMsdcUsgMNUser
 ******************************************************************************/

PnlMsdcUsgMNUser::PnlMsdcUsgMNUser(
			XchgMsdc* xchg
			, DbsMsdc* dbsmsdc
			, const ubigint jrefSup
			, const uint ixMsdcVLocale
		) :
			JobMsdc(xchg, VecMsdcVJob::PNLMSDCUSGMNUSER, jrefSup, ixMsdcVLocale)
		{
	...
	qry = new QryMsdcUsgMNUser(xchg, dbsmsdc, jref, ixMsdcVLocale);
...
// IP cust --- INSERT
DpchEngMsdc* PnlMsdcUsgMNUser::getNewDpchEng(
			set<uint> items
		) {
...
void PnlMsdcUsgMNUser::updatePreset(
			DbsMsdc* dbsmsdc
...
void PnlMsdcUsgMNUser::handleRequest(
			DbsMsdc* dbsmsdc
			, ReqMsdc* req
		) {
	if (req->ixVBasetype == ReqMsdc::VecVBasetype::CMD) {
		...
		} else if (req->dpchapp->ixMsdcVDpch == VecMsdcVDpch::DPCHAPPMSDCUSGMNUSERDATA) {
		...
		} else if (req->dpchapp->ixMsdcVDpch == VecMsdcVDpch::DPCHAPPMSDCUSGMNUSERDO) {
...
void PnlMsdcUsgMNUser::handleDpchAppMsdcInit(
			DbsMsdc* dbsmsdc
...
void PnlMsdcUsgMNUser::handleDpchAppDataStgiac(
			DbsMsdc* dbsmsdc
			, StgIac* _stgiac
			, DpchEngMsdc** dpcheng
		) {
...
void PnlMsdcUsgMNUser::handleDpchAppDataStgiacqry(
			DbsMsdc* dbsmsdc
			, QryMsdcUsgMNUser::StgIac* _stgiacqry
			, DpchEngMsdc** dpcheng
		) {
	set<uint> diffitems;
	set<uint> moditems;
	...
	if (!diffitems.empty()) {
		...
		if (has(diffitems, QryMsdcUsgMNUser::StgIac::JNUM)) recSelNew = qry->getRecByJnum(_stgiacqry->jnum);

		if (!has(diffitems, QryMsdcUsgMNUser::StgIac::JNUM) || (diffitems.size() > 1)) {
			qry->rerun(dbsmsdc);
			insert(moditems, {DpchEngData::STATSHRQRY, DpchEngData::RST});
		};

		if (has(diffitems, QryMsdcUsgMNUser::StgIac::JNUM)) {
			...
			if (recSelNew) {
				...
				if (dbsmsdc->tblmsdcmuser->loadRecByRef(recSelNew->mref, &_recUsr)) {
					recUsr = *_recUsr;
					delete _recUsr;
				};
			};
...
void PnlMsdcUsgMNUser::handleDpchAppDoButViewClick(
			DbsMsdc* dbsmsdc
			, DpchEngMsdc** dpcheng
		) {
	ubigint jrefNew = 0;
	...
	if (statshr.ButViewAvail && statshr.ButViewActive) {
		if (xchg->getIxPreset(VecMsdcVPreset::PREMSDCIXCRDACCUSR, jref)) {
			sref = "CrdMsdcUsr";
			xchg->triggerIxRefSrefIntvalToRefCall(dbsmsdc, VecMsdcVCall::CALLMSDCCRDOPEN, jref, 0, 0, sref, recUsr.ref, jrefNew);
		};
...
void PnlMsdcUsgMNUser::handleDpchAppDoButAddClick(
			DbsMsdc* dbsmsdc
			, DpchEngMsdc** dpcheng
		) {
...
void PnlMsdcUsgMNUser::handleDpchAppDoButSubClick(
			DbsMsdc* dbsmsdc
			, DpchEngMsdc** dpcheng
		) {
...
void PnlMsdcUsgMNUser::handleDpchAppDoButRefreshClick(
			DbsMsdc* dbsmsdc
			, DpchEngMsdc** dpcheng
		) {
	set<uint> moditems;
...
void PnlMsdcUsgMNUser::handleCall(
			DbsMsdc* dbsmsdc
			, Call* call
		) {
...
bool PnlMsdcUsgMNUser::handleCallMsdcStatChg(
			DbsMsdc* dbsmsdc
			, const ubigint jrefTrig
		) {
	bool retval = false;
