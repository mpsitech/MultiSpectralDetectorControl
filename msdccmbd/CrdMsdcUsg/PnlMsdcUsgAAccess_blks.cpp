/**
  * \file PnlMsdcUsgAAccess_blks.cpp
  * job handler for job PnlMsdcUsgAAccess (implementation of blocks)
...
void PnlMsdcUsgAAccess::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfMsdcUsgAAccess";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfMsdcUsgAAccess";
...
void PnlMsdcUsgAAccess::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const uint ixMsdcVExpstate
		) {
	if (difftag.length() == 0) difftag = "StatAppMsdcUsgAAccess";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppMsdcUsgAAccess";
...
void PnlMsdcUsgAAccess::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrMsdcUsgAAccess";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrMsdcUsgAAccess";
...
void PnlMsdcUsgAAccess::Tag::writeXML(
			const uint ixMsdcVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagMsdcUsgAAccess";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemMsdcUsgAAccess";
...
PnlMsdcUsgAAccess::DpchAppData::DpchAppData() : DpchAppMsdc(VecMsdcVDpch::DPCHAPPMSDCUSGAACCESSDATA) {
};
...
void PnlMsdcUsgAAccess::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppMsdcUsgAAccessData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlMsdcUsgAAccess::DpchAppDo::DpchAppDo() : DpchAppMsdc(VecMsdcVDpch::DPCHAPPMSDCUSGAACCESSDO) {
	ixVDo = 0;
};
...
void PnlMsdcUsgAAccess::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppMsdcUsgAAccessDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlMsdcUsgAAccess::DpchEngData::DpchEngData(
			...
			, const set<uint>& mask
		) : DpchEngMsdc(VecMsdcVDpch::DPCHENGMSDCUSGAACCESSDATA, jref) {
...
void PnlMsdcUsgAAccess::DpchEngData::merge(
			DpchEngMsdc* dpcheng
		) {
...
void PnlMsdcUsgAAccess::DpchEngData::writeXML(
			const uint ixMsdcVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngMsdcUsgAAccessData");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/msdc");
		...
		if (has(STATAPPQRY)) QryMsdcUsgAAccess::StatApp::writeXML(wr);
