/**
  * \file PnlMsdcPrsRec_blks.cpp
  * job handler for job PnlMsdcPrsRec (implementation of blocks)
...
void PnlMsdcPrsRec::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfMsdcPrsRec";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfMsdcPrsRec";
...
void PnlMsdcPrsRec::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
			, const bool initdoneDetail
		) {
	if (difftag.length() == 0) difftag = "StatAppMsdcPrsRec";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppMsdcPrsRec";
...
void PnlMsdcPrsRec::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrMsdcPrsRec";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrMsdcPrsRec";
...
void PnlMsdcPrsRec::Tag::writeXML(
			const uint ixMsdcVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagMsdcPrsRec";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemMsdcPrsRec";
...
PnlMsdcPrsRec::DpchAppDo::DpchAppDo() :
			DpchAppMsdc(VecMsdcVDpch::DPCHAPPMSDCPRSRECDO)
		{
	ixVDo = 0;
};
...
void PnlMsdcPrsRec::DpchAppDo::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppMsdcPrsRecDo");
	else
...
PnlMsdcPrsRec::DpchEngData::DpchEngData(
			...
			DpchEngMsdc(VecMsdcVDpch::DPCHENGMSDCPRSRECDATA, jref)
		{
...
void PnlMsdcPrsRec::DpchEngData::merge(
			DpchEngMsdc* dpcheng
		) {
...
void PnlMsdcPrsRec::DpchEngData::writeXML(
			const uint ixMsdcVLocale
			, xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngMsdcPrsRecData");
