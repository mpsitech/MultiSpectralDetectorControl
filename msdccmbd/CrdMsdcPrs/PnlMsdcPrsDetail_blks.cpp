/**
  * \file PnlMsdcPrsDetail_blks.cpp
  * job handler for job PnlMsdcPrsDetail (implementation of blocks)
...
void PnlMsdcPrsDetail::ContInf::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "ContInfMsdcPrsDetail";

	string itemtag;
	if (shorttags) itemtag = "Ci";
	else itemtag = "ContitemInfMsdcPrsDetail";
...
void PnlMsdcPrsDetail::StatApp::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
	...
	if (difftag.length() == 0) difftag = "StatAppMsdcPrsDetail";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemAppMsdcPrsDetail";
...
void PnlMsdcPrsDetail::StatShr::writeXML(
			xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "StatShrMsdcPrsDetail";

	string itemtag;
	if (shorttags) itemtag = "Si";
	else itemtag = "StatitemShrMsdcPrsDetail";
...
void PnlMsdcPrsDetail::Tag::writeXML(
			const uint ixMsdcVLocale
			, xmlTextWriter* wr
			, string difftag
			, bool shorttags
		) {
	if (difftag.length() == 0) difftag = "TagMsdcPrsDetail";

	string itemtag;
	if (shorttags) itemtag = "Ti";
	else itemtag = "TagitemMsdcPrsDetail";
...
PnlMsdcPrsDetail::DpchAppData::DpchAppData() : DpchAppMsdc(VecMsdcVDpch::DPCHAPPMSDCPRSDETAILDATA) {
};
...
void PnlMsdcPrsDetail::DpchAppData::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppMsdcPrsDetailData");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlMsdcPrsDetail::DpchAppDo::DpchAppDo() : DpchAppMsdc(VecMsdcVDpch::DPCHAPPMSDCPRSDETAILDO) {
	ixVDo = 0;
};
...
void PnlMsdcPrsDetail::DpchAppDo::readXML(
			pthread_mutex_t* mScr
			, map<string,ubigint>& descr
			, xmlXPathContext* docctx
			, string basexpath
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchAppMsdcPrsDetailDo");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlMsdcPrsDetail::DpchEngData::DpchEngData(
			const ubigint jref
			, ContIac* contiac
			, ContInf* continf
			, Feed* feedFLstDrv
			...
			, StatShr* statshr
			, const set<uint>& mask
		) : DpchEngMsdc(VecMsdcVDpch::DPCHENGMSDCPRSDETAILDATA, jref) {
...
void PnlMsdcPrsDetail::DpchEngData::merge(
			DpchEngMsdc* dpcheng
		) {
...
void PnlMsdcPrsDetail::DpchEngData::writeXML(
			const uint ixMsdcVLocale
			, pthread_mutex_t* mScr
	...
	xmlTextWriterStartElement(wr, BAD_CAST "DpchEngMsdcPrsDetailData");
