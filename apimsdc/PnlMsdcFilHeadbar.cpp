/**
  * \file PnlMsdcFilHeadbar.cpp
  * API code for job PnlMsdcFilHeadbar (implementation)
...
PnlMsdcFilHeadbar::StgInf::StgInf(
...
bool PnlMsdcFilHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfMsdcFilHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfMsdcFilHeadbar";
...
set<uint> PnlMsdcFilHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlMsdcFilHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlMsdcFilHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagMsdcFilHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemMsdcFilHeadbar";
...
PnlMsdcFilHeadbar::DpchEngData::DpchEngData() : DpchEngMsdc(VecMsdcVDpch::DPCHENGMSDCFILHEADBARDATA) {
};
...
void PnlMsdcFilHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngMsdcFilHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
