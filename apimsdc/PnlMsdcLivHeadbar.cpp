/**
  ...
  * API code for job PnlMsdcLivHeadbar (implementation)
...
PnlMsdcLivHeadbar::StgInf::StgInf(
...
bool PnlMsdcLivHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfMsdcLivHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfMsdcLivHeadbar";
...
set<uint> PnlMsdcLivHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlMsdcLivHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlMsdcLivHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagMsdcLivHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemMsdcLivHeadbar";
...
PnlMsdcLivHeadbar::DpchEngData::DpchEngData() : DpchEngMsdc(VecMsdcVDpch::DPCHENGMSDCLIVHEADBARDATA) {
};
...
void PnlMsdcLivHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngMsdcLivHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
