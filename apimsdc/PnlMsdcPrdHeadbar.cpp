...
PnlMsdcPrdHeadbar::StgInf::StgInf(
...
bool PnlMsdcPrdHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfMsdcPrdHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfMsdcPrdHeadbar";
...
set<uint> PnlMsdcPrdHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlMsdcPrdHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlMsdcPrdHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagMsdcPrdHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemMsdcPrdHeadbar";
...
void PnlMsdcPrdHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngMsdcPrdHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
