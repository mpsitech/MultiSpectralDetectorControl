...
bool CrdMsdcUsr::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfMsdcUsr");
	else
...
bool CrdMsdcUsr::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppMsdcUsr");
	else
		basefound = checkXPath(docctx, basexpath);
...
set<uint> CrdMsdcUsr::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> CrdMsdcUsr::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool CrdMsdcUsr::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrMsdcUsr");
	else
		basefound = checkXPath(docctx, basexpath);
...
bool CrdMsdcUsr::Tag::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagMsdcUsr");
	else
		basefound = checkXPath(docctx, basexpath);
...
CrdMsdcUsr::DpchAppDo::DpchAppDo(
			const string& scrJref
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppMsdc(VecMsdcVDpch::DPCHAPPMSDCUSRDO, scrJref) {
...
void CrdMsdcUsr::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppMsdcUsrDo");
...
CrdMsdcUsr::DpchEngData::DpchEngData() : DpchEngMsdc(VecMsdcVDpch::DPCHENGMSDCUSRDATA) {
...
void CrdMsdcUsr::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngMsdcUsrData");
	else
		basefound = checkXPath(docctx, basexpath);
