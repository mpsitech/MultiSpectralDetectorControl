/**
  * \file PnlMsdcDatHeadbar.cpp
  * API code for job PnlMsdcDatHeadbar (implementation)
...
PnlMsdcDatHeadbar::StgInf::StgInf(
...
bool PnlMsdcDatHeadbar::StgInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StgInfMsdcDatHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StgitemInfMsdcDatHeadbar";
...
set<uint> PnlMsdcDatHeadbar::StgInf::comm(
			const StgInf* comp
		) {
	set<uint> items;
...
set<uint> PnlMsdcDatHeadbar::StgInf::diff(
			const StgInf* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlMsdcDatHeadbar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagMsdcDatHeadbar");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemMsdcDatHeadbar";
...
PnlMsdcDatHeadbar::DpchEngData::DpchEngData() : DpchEngMsdc(VecMsdcVDpch::DPCHENGMSDCDATHEADBARDATA) {
};
...
void PnlMsdcDatHeadbar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngMsdcDatHeadbarData");
	else
		basefound = checkXPath(docctx, basexpath);
