/**
  * \file CrdMsdcDat.cpp
  * API code for job CrdMsdcDat (implementation)
...
bool CrdMsdcDat::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfMsdcDat");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfMsdcDat";

	if (basefound) {
		if (extractUintAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "numFSge", numFSge)) add(NUMFSGE);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MrlAppHlp", MrlAppHlp)) add(MRLAPPHLP);
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ci", "sref", "MtxCrdDat", MtxCrdDat)) add(MTXCRDDAT);
	};
...
bool CrdMsdcDat::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppMsdcDat");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppMsdcDat";
...
set<uint> CrdMsdcDat::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> CrdMsdcDat::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool CrdMsdcDat::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrMsdcDat");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrMsdcDat";
...
bool CrdMsdcDat::Tag::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagMsdcDat");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemMsdcDat";
...
CrdMsdcDat::DpchAppDo::DpchAppDo(
			...
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppMsdc(VecMsdcVDpch::DPCHAPPMSDCDATDO, scrJref) {
...
void CrdMsdcDat::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppMsdcDatDo");
...
CrdMsdcDat::DpchEngData::DpchEngData() : DpchEngMsdc(VecMsdcVDpch::DPCHENGMSDCDATDATA) {
...
void CrdMsdcDat::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngMsdcDatData");
	else
		basefound = checkXPath(docctx, basexpath);
