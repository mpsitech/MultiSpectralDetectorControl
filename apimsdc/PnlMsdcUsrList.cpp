/**
  ...
  * API code for job PnlMsdcUsrList (implementation)
...
bool PnlMsdcUsrList::ContInf::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfMsdcUsrList");
	else
		basefound = checkXPath(docctx, basexpath);
...
bool PnlMsdcUsrList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrMsdcUsrList");
	else
		basefound = checkXPath(docctx, basexpath);
...
set<uint> PnlMsdcUsrList::StgIac::diff(
	...
	diffitems = {, TCOSRFWIDTH, TCOUSGWIDTH, TCOSTEWIDTH, TCOLCLWIDTH, TCOULVWIDTH};
	for (auto it=commitems.begin();it!=commitems.end();it++) diffitems.erase(*it);
...
bool PnlMsdcUsrList::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagMsdcUsrList");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlMsdcUsrList::DpchAppData::DpchAppData(
			...
			DpchAppMsdc(VecMsdcVDpch::DPCHAPPMSDCUSRLISTDATA, scrJref)
		{
...
void PnlMsdcUsrList::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppMsdcUsrListData");
...
PnlMsdcUsrList::DpchAppDo::DpchAppDo(
			...
			DpchAppMsdc(VecMsdcVDpch::DPCHAPPMSDCUSRLISTDO, scrJref)
		{
...
void PnlMsdcUsrList::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppMsdcUsrListDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/msdc");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlMsdcUsrList::DpchEngData::DpchEngData() :
			DpchEngMsdc(VecMsdcVDpch::DPCHENGMSDCUSRLISTDATA)
		{
...
void PnlMsdcUsrList::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngMsdcUsrListData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryMsdcUsrList::StatApp();
		statshrqry = QryMsdcUsrList::StatShr();
