...
QryMsdcUsrList::StatApp::StatApp(
			const uint firstcol
...
bool QryMsdcUsrList::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryMsdcUsrList");
	else
...
set<uint> QryMsdcUsrList::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryMsdcUsrList::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryMsdcUsrList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryMsdcUsrList");
	else
		basefound = checkXPath(docctx, basexpath);
