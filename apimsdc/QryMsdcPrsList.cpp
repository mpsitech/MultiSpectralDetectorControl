/**
  ...
  * API code for job QryMsdcPrsList (implementation)
...
QryMsdcPrsList::StatApp::StatApp(
			const uint firstcol
...
bool QryMsdcPrsList::StatApp::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppQryMsdcPrsList");
	else
...
set<uint> QryMsdcPrsList::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> QryMsdcPrsList::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool QryMsdcPrsList::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrQryMsdcPrsList");
	else
		basefound = checkXPath(docctx, basexpath);
