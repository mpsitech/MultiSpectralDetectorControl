/**
  * \file CrdMsdcUsg.cpp
  * API code for job CrdMsdcUsg (implementation)
...
bool CrdMsdcUsg::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfMsdcUsg");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "ContitemInfMsdcUsg";
...
bool CrdMsdcUsg::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppMsdcUsg");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemAppMsdcUsg";
...
set<uint> CrdMsdcUsg::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> CrdMsdcUsg::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool CrdMsdcUsg::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrMsdcUsg");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "StatitemShrMsdcUsg";
...
bool CrdMsdcUsg::Tag::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagMsdcUsg");
	else
		basefound = checkXPath(docctx, basexpath);

	string itemtag = "TagitemMsdcUsg";
...
CrdMsdcUsg::DpchAppDo::DpchAppDo(
			...
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppMsdc(VecMsdcVDpch::DPCHAPPMSDCUSGDO, scrJref) {
...
void CrdMsdcUsg::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppMsdcUsgDo");
...
CrdMsdcUsg::DpchEngData::DpchEngData() : DpchEngMsdc(VecMsdcVDpch::DPCHENGMSDCUSGDATA) {
...
void CrdMsdcUsg::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngMsdcUsgData");
	else
		basefound = checkXPath(docctx, basexpath);
