...
bool PnlMsdcDatAPar::ContInf::readXML(
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "ContInfMsdcDatAPar");
	else
...
PnlMsdcDatAPar::StatApp::StatApp(
...
bool PnlMsdcDatAPar::StatApp::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatAppMsdcDatAPar");
	else
		basefound = checkXPath(docctx, basexpath);
...
set<uint> PnlMsdcDatAPar::StatApp::comm(
			const StatApp* comp
		) {
	set<uint> items;
...
set<uint> PnlMsdcDatAPar::StatApp::diff(
			const StatApp* comp
		) {
	set<uint> commitems;
	set<uint> diffitems;
...
bool PnlMsdcDatAPar::StatShr::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "StatShrMsdcDatAPar");
	else
		basefound = checkXPath(docctx, basexpath);
...
PnlMsdcDatAPar::Tag::Tag(
	...
	mask = {CPT, TRS, TXTSHOWING1, TXTSHOWING2, TCOSTA, TCOKEY, TCOFIL, TCOVAL, TCOBIN};
};

bool PnlMsdcDatAPar::Tag::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "TagMsdcDatAPar");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		if (extractStringAttrUclc(docctx, basexpath, itemtag, "Ti", "sref", "TcoBin", TcoBin)) add(TCOBIN);
	};
...
PnlMsdcDatAPar::DpchAppData::DpchAppData(
			...
			, const set<uint>& mask
		) : DpchAppMsdc(VecMsdcVDpch::DPCHAPPMSDCDATAPARDATA, scrJref) {
...
void PnlMsdcDatAPar::DpchAppData::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppMsdcDatAParData");
...
PnlMsdcDatAPar::DpchAppDo::DpchAppDo(
			...
			, const uint ixVDo
			, const set<uint>& mask
		) : DpchAppMsdc(VecMsdcVDpch::DPCHAPPMSDCDATAPARDO, scrJref) {
...
void PnlMsdcDatAPar::DpchAppDo::writeXML(
			xmlTextWriter* wr
		) {
	xmlTextWriterStartElement(wr, BAD_CAST "DpchAppMsdcDatAParDo");
	xmlTextWriterWriteAttribute(wr, BAD_CAST "xmlns", BAD_CAST "http://www.mpsitech.com/msdc");
		if (has(SCRJREF)) writeString(wr, "scrJref", scrJref);
...
PnlMsdcDatAPar::DpchEngData::DpchEngData() : DpchEngMsdc(VecMsdcVDpch::DPCHENGMSDCDATAPARDATA) {
...
void PnlMsdcDatAPar::DpchEngData::readXML(
			xmlXPathContext* docctx
			, string basexpath
			, bool addbasetag
		) {
	...
	if (addbasetag)
		basefound = checkUclcXPaths(docctx, basexpath, basexpath, "DpchEngMsdcDatAParData");
	else
		basefound = checkXPath(docctx, basexpath);
	...
	if (basefound) {
		...
		tag = Tag();
		rst.clear();
		statappqry = QryMsdcDatAPar::StatApp();
		statshrqry = QryMsdcDatAPar::StatShr();
